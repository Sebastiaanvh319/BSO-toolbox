// Visualization BSD and Conformal model  
...
// --- predictionCF_ML Function --- 
//     : create multidimensional vector with predicted coordinates of conformal model
vector<vector<vector<float>>> predictionCF_ML()   
{
    // Load predicted data
    ...
    return ncoordinates;            // return coordinates to use in VISUALIZATION
}
vector<float> allvertices()   
{
    ...
    return predictedCoordinatesFl;            // return coordinates to use in VISUALIZATION
}
...
void drawVertices()
{
    vector<float> coorN = allvertices(); 
    int norm = 10000;                                    
...
{
        vector<vector<vector<float>>> coorN = predictionCF_ML();                     // call coordinates vector from predictionCF_ML() Function
        int norm = 10000;                                      // divide by 10000, all numbers between 0 and 1 (normalized)
        for (int c = 0; c < coorN.size(); c++)
        {
            for (int p = 0; p < 8; p++)
            {
        ...
        {    
            for (int p = 0; p < 8; p++)
            {
                glVertex3f(coorN[c][p][x],coorN[c][p][y],-coorN[c][p][z]);
            }
            for (int p = 0; p < 4; p++)
            {
                if (p < 3){
                glVertex3f(coorN[c][p][x],coorN[c][p][y],-coorN[c][p][z]);
                glVertex3f(coorN[c][p+1][x],coorN[c][p+1][y],-coorN[c][p+1][z]);
                glVertex3f(coorN[c][p+5][x],coorN[c][p+5][y],-coorN[c][p+5][z]);
                glVertex3f(coorN[c][p+4][x],coorN[c][p+4][y],-coorN[c][p+4][z]);     
                } else {
                glVertex3f(coorN[c][p][x],coorN[c][p][y],-coorN[c][p][z]);
                glVertex3f(coorN[c][p+1-4][x],coorN[c][p+1-4][y],-coorN[c][p+1-4][z]);
                glVertex3f(coorN[c][p+5-4][x],coorN[c][p+5-4][y],-coorN[c][p+5-4][z]);
                glVertex3f(coorN[c][p+4][x],coorN[c][p+4][y],-coorN[c][p+4][z]);                     
                }           
            }
        }
...
{
        vector<vector<vector<float>>> coorN = predictionCF_ML();                     // call coordinates vector from predictionCF_ML() Function
        float norm = 10000;                                      // divide by 10000, all numbers between 0 and 1 (normalized)
        for (int c = 0; c < coorN.size(); c++)
        {
            for (int p = 0; p < 8; p++)
            {
